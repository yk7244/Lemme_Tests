...
def send_and_receive_serial_data(data, port='/dev/ttyTHS1', baudrate=9600):
    """
    Function to send data over UART and receive the response (loopback).
    
    Parameters:
    - data: The data to be sent as a string.
    - port: The UART device to use (default '/dev/ttyTHS1').
    ...
    try:
        with serial.Serial(port, baudrate, timeout=1) as ser:
            ...
            # Wait briefly to allow data to loop back
            time.sleep(0.1)
            ...
            if response:
                ...
                print("No data received (check loopback setup).")
            return response
    except serial.SerialException as e:
        print("Error in UART communication:", e)
        return None

# Main loop to repeatedly detect wake word, listen, transcribe, and send/receive data
    ...
    while True:
        ...
        if wake_up_word_detection():
            ...
            if pcm_data is not None:
                ...
                # Step 3: Send and receive data via UART loopback
                response = send_and_receive_serial_data(text, port='/dev/ttyTHS1', baudrate=9600)
                if response:
                    print(f"Loopback successful: {response}")
                else:
